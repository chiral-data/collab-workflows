...
def main():
    ...
    # Post-process each swarm: generate conformations and cluster
    ...
    # Rank all poses across swarms
    print("\nRanking poses...", flush=True)
    run_cmd(["lgd_rank.py", str(num_swarms), str(num_conformations)], check=False)

    # Collect top poses: prefer rank.list, fall back to reading gso files directly
    top_poses = []
        ...
        with open("rank.list") as f:
            for line in f:
                ...
                if not line or line.startswith("#") or not line[0].isdigit():
                    continue
                parts = line.split()
                # rank.list format: rank  score  cluster_size  pdb_path (varies by version)
                # Try to extract a score and a pdb reference
                try:
                    # Attempt: first numeric = rank, second = score
                    score = float(parts[1]) if len(parts) > 1 else float(parts[0])
                    # Look for a path like swarm_N/lightdock_M.pdb in the line
    ...
    # Fallback: collect best pose per swarm by parsing gso output scores
    if not top_poses:
    ...
    # Build multi-MODEL top_predictions.pdb
    ...
    print(f"Saved top_predictions.pdb and rank_results.json", flush=True)
    if top_poses:
