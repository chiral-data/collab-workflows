...
def read_first_model_pdb(path):
    """Return ATOM/HETATM lines from the first MODEL in a multi-model PDB, or all if no MODEL records."""
    lines = []
    in_model = False
    has_model = False
...
def main():
    ...
    top_pdb_content = read_first_model_pdb(pdb_path)

    # Build HTML rows for the ranking table
    table_rows = ""
...
(function() {{
  ...
  viewer.setStyle({{ chain: 'A' }}, {{ cartoon: {{ color: '#3498db' }} }});
  viewer.setStyle({{ chain: 'B' }}, {{ cartoon: {{ color: '#e74c3c' }} }});
  // Fallback: colour all if single chain
