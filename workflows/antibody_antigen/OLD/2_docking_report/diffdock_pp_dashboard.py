...
class DiffDockPPResultsParser:
    ...
    def _parse_results(self):
        ...
        # Export poses to PDB files - DISABLED: Using separate protein-only extraction script
        # self._export_poses_to_pdb(predictions_data)
        
        # Parse summary file for metadata
        metadata = self._parse_summary_file()
        
        if not poses:
            # Create dummy poses for demonstration if no poses found
            poses = self._create_demo_poses()
            print("Warning: No poses extracted from pickle files. Using demo data.")
...
class DiffDockPPDashboard:
    ...
    def __init__(self, results: DiffDockPPResults):
...
<body>
    ...
    <div class="plot-container">
        ...
        <p><strong>PDB Files Available:</strong> Check the <code>diffdock_pp_results/pdb_poses/</code> directory for PDB files.</p>
        ...
        <ul>
            <li><code>pose_1_complex.pdb</code> - Complete protein-protein complex</li>
            <li><code>pose_1_receptor.pdb</code> - Receptor structure (Chain A)</li>
            <li><code>pose_1_ligand.pdb</code> - Ligand structure (Chain B)</li>
        </ul>
    </div>
...
def main():
    ...
    try:
        ...
        # Create dashboard
        dashboard = DiffDockPPDashboard(parser.results)
