...
Generates interactive HTML visualization: Enhanced Single-Panel Dashboard
Inspired by Node 7 design (Clustering, Search, Filter).
"""
...
def generate_coherence_html(json_filename='coherence_data.json'):
    """Generate HTML for Pathway Coherence visualization (Enhanced + Pairwise)"""
...
<body>
    <div class="container">
        <header>
            ...
            <p class="subtitle">Interactive Clustered Networks</p>
        </header>
    ...
    <script>
        ...
        // Custom Colorscale for Coherence
        const COLORSCALE = [
            [0.0, '#ffffd9'],
            [0.2, '#c7e9b4'],
            [0.4, '#41b6c4'],
        ...
        function init() {
            ...
            // Set Defaults (Try to pick 'Control' or 'RRMS' first)
            if (COHORTS.includes('Control')) select.value = 'Control';
            else if (COHORTS.length > 0) select.value = COHORTS[0];
        ...
        function updateView() {
            ...
            document.getElementById('plot-title').textContent = cohortName + " Network";
        }
        ...
        function renderHeatmap(divId, cohortName, threshold, showDendro) {
            ...
            [1, 2].forEach(id => {
                ...
                if (term) {
                    ...
                    if (idx !== -1) {
                        const color = id === 1 ? '#ff00ff' : '#00ffff'; 
            ...
            const layout = {
                ...
                xaxis: { 
                    tickangle: -45, 
                    tickfont: { size: 10 },
                    domain: (showDendro && data.dendrogram) ? [0.15, 1] : [0, 1],
                    anchor: 'y'
                },
                yaxis: { 
                    automargin: true, 
                    autorange: 'reversed', 
                    tickfont: { size: 10 },
                    domain: (showDendro && data.dendrogram) ? [0, 0.85] : [0, 1],
                    anchor: 'x',
                    side: 'right'
                },
                xaxis2: { // Top Dendro
                    domain: (showDendro && data.dendrogram) ? [0.15, 1] : [0, 0], 
                    anchor: 'y2', showgrid: false, zeroline: false, showticklabels: false
                },
                yaxis2: { 
                    domain: (showDendro && data.dendrogram) ? [0.85, 1] : [1, 1], 
                    anchor: 'x2', showgrid: false, zeroline: false, showticklabels: false
                },
                xaxis3: { // Left Dendro
                    domain: (showDendro && data.dendrogram) ? [0, 0.15] : [0, 0], 
                    anchor: 'y3', showgrid: false, zeroline: false, showticklabels: false, autorange: 'reversed'
                },
                yaxis3: { 
                    domain: (showDendro && data.dendrogram) ? [0, 0.85] : [0, 0], 
