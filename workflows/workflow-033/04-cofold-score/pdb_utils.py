...
- map PDB author residue numbers -> 1-based sequence index (hotspot remap)
- CA coordinates for RMSD

Ported from NVIDIA BioNeMo Agent Toolkit (Apache-2.0 / CC-BY-4.0).
"""
...
def _iter_atom_lines(pdb_text: str, chain: str | None = None):
...
def extract_chain(pdb_text: str, chain: str) -> str:
    """Return PDB text containing only ATOM/HETATM/TER records for ``chain``."""
    keep = []
    for line in pdb_text.splitlines():
...
def ca_residues(pdb_text: str, chain: str | None = None) -> list[tuple]:
...
def sequence(pdb_text: str, chain: str | None = None) -> str:
...
def residue_index_map(pdb_text: str, chain: str | None = None) -> dict[str, int]:
    """Map PDB author residue id → 1-based sequence index (CA order).

    Keys stored both as bare number ('56') and, when an insertion code is
    present, as number+icode ('56A').
    """
...
def remap_to_seq_index(pdb_text: str, chain: str, author_resnums: list[str]) -> list[int]:
...
def ca_coords(pdb_text: str, chain: str | None = None) -> list[tuple[float, float, float]]:
