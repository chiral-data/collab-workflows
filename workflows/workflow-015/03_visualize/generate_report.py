    ...
    for mode, preds in sorted(modes_data.items()):
        ...
        for p in preds:
            ...
            rows_html.append(
                ...
                f"<td><code>{html.escape(p['go_id'])}</code></td>"
                f"<td>{html.escape(p['go_name'])}</td>"
...
<div class="card mb-3">
  <div class="card-header d-flex justify-content-between align-items-center">
    <strong>{html.escape(protein)}</strong>
...
<div class="container py-4">
  ...
  <footer class="text-center text-muted mt-4 small">
    Generated by mDeepFRI silva workflow &bull; Results from <code>{html.escape(os.path.basename(results_path))}</code>
  </footer>
</div>
</body>
</html>"""
...
def main():
    ...
    html_content = build_html(by_protein, align_lookup, args.min_score, args.top_n, RESULTS_PATH)
